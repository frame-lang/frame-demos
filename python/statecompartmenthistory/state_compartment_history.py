...
class HistoryStateContext:
    ...
    def __init__(self, ):
        ...
        self.compartment = HistoryStateContextCompartment(self.state)
    ...
    def _sStart_(self, e):
        if e._message == ">>":
            compartment = HistoryStateContextCompartment(self._sS0_)
    ...
    def _sS0_(self, e):
        if e._message == ">":
            self.print_do((self.compartment.state_vars["enterMsg"]))
            return
        
        elif e._message == "switchState":
            # Switch\nState
            compartment = HistoryStateContextCompartment(self._sS1_)
            self.transition(compartment)
            return
            ...
            # Goto\nDead End
            compartment = HistoryStateContextCompartment(self._sDeadEnd_)
            self.transition(compartment)
            return
